...
export const TimezoneDisplay: FC<Props> = ({ parsedDate, use24HourFormat, sortTimezonesByTime }) => {
  const [favoriteTimezones, setFavoriteTimezones] = useState<string[]>(() => JSON.parse(localStorage.getItem("favoriteTimezones") ?? "[]"));
  const [filter, setFilter] = useState(() => localStorage.getItem("timezoneFilter") ?? "");
  ...
  return (
    <details>
      ...
      <summary className="cursor-pointer font-semibold underline select-none">Timezones</summary>
        ...
        {filteredTimezones.map(timezone => {
          ...
          return (
            <button
              ...
              className="group border-l-2 border-transparent p-1.5 px-2 text-left hover:border-l-black hover:bg-neutral-200 data-[users-timezone=true]:border-l-4 data-[users-timezone=true]:border-black dark:hover:border-l-white dark:hover:bg-neutral-900 dark:data-[users-timezone=true]:border-white"
